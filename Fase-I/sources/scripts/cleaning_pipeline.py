...
def filtrar_por_idioma(
    ...
    """
    Detecta el idioma de cada review y conserva solo las del idioma indicado
    con confianza suficiente. Elimina las dudosas y las de otros idiomas.
    """
    ...
    df = df.drop(columns=["idioma", "confianza", "categoria_idioma"])
...
def run_pipeline(
    ...
    Retorna
    -------
    pd.DataFrame limpio y listo para entrenamiento.
    """
    print(f"\n{'='*55}")
    ...
    print("\n[3/8] Detectando y filtrando por idioma...")
    df = filtrar_por_idioma(df, col_texto, umbral_confianza_idioma, solo_idioma)
